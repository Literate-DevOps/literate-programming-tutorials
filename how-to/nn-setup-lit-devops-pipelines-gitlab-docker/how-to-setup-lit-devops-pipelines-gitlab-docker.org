...
5. Verify that local ports are not in use before running the docker container.
   ...
   - For Unix, Linux and MacOS users
     ...
     #+begin_quote
       *Important:* If output shows any network ports are already in use, then you will need to pick an new port numbers and manually adjust the ~docker run~ command parameters in future tutorial steps as a workaround for your specific setup.
     #+end_quote
