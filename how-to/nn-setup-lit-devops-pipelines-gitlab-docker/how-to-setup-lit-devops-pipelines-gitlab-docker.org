...
5. Verify that local ports are not in use before running the docker container.
   ...
   - For Unix, Linux and MacOS users
     ...
     #+begin_src ruby :var data=gitlab-ce-local-port-mappings :wrap "quote" :exports results :results org replace 
      " *Note:* For this tutorial, we need to verify that network ports =%s=, =%s= and =%s= are not in use. " % data.map{|d| d[0]}
     #+end_src
